...
const getSearchParam = (searchParams: Record<string, SearchParamValue>, key: string) => {
  const value = searchParams[key];
  return Array.isArray(value) ? value[0] : value ?? "";
};

const HomePageClient = ({ searchParams }: HomePageClientProps) => {
  const router = useRouter();
  const search = getSearchParam(searchParams, "search");
  const categoryId = getSearchParam(searchParams, "categoryId");
  const brands = getSearchParam(searchParams, "brands");
  const minPrice = getSearchParam(searchParams, "minPrice");
  const maxPrice = getSearchParam(searchParams, "maxPrice");
  const ratingMin = getSearchParam(searchParams, "ratingMin");
  const freeDelivery = getSearchParam(searchParams, "freeDelivery");
  const condition = getSearchParam(searchParams, "condition");
  const sort = getSearchParam(searchParams, "sort") || "featured";
  ...
  const { products, total, page, limit, setPage, isLoading, error } = useProducts({
    ...
    ratingMin,
    freeDelivery,
    discount: getSearchParam(searchParams, "discount"),
    condition,
    sort
  });
  ...
  const isSearchView = Boolean(
    search ||
      ...
      freeDelivery ||
      condition ||
      getSearchParam(searchParams, "discount")
  );
